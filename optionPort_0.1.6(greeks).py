...
simulatedPrice = np.linspace(900, 1200, points) #coulndt be 0
...
def getOptPremium(cost, St):
    return  cost + St*0
class d1:
    def __init__(self,St, k, sigma, rf, div, T, t):
        self.St = St
        self.k = k
        self.sigma = sigma
        self.rf = rf
        ...
        self.t = t
    def dOne(self):
        return ( math.log(self.St/self.k) + (self.rf +self.div + math.pow( self.sigma, 2)/2 ) * (self.T-self.t) ) / ( self.sigma * math.sqrt((self.T-self.t)) )
class d2(d1):
    def __init__(self, sigma, T, t):
        super(d2, self).__init__(St, k, sigma, rf, div, T, t)
    def dTwo(self):
        return self.dOne - self.sigma * math.sqrt((self.T- self.t))

class greek(d1,d2):
    def __init__(self, St, k, sigma, rf, div, T, t, right):
        super(greek, self).__init__(St, k, sigma, rf, div, T, t)
        self.right = right
    def get_price_delta(self):
        if self.right == 'C':
            calc_price = ( norm.cdf(d1.dOne) * self.St * math.exp(-self.div * (self.t-self.T))  - norm.cdf(d2.dTwo) * self.k * math.exp(-self.rf * (self.T-self.t) ) )
            delta = norm.cdf(d1.dOne)
        elif self.right == 'P':
            calc_price =  (-norm.cdf(-d1.dOne) * self.St * math.exp(-self.div* (self.t-self.T)) + norm.cdf(-d2.dTwo) * self.k * math.exp( -self.rf * (self.t-self.T) )) 
            delta = -norm.cdf(-d1.dOne) 
...
# def get_price_delta(d1,d2,St, k, t, T, rf, sigma,right,div):
...
for k in callOptStrike:
    for St in simulatedPrice:
        callOptPayoff.append(greek.get_price_delta(d2,St,int(k),t,T,rf,sigma,'C',div))
...
for k in callOptStrike:
    for St in simulatedPrice:
        callOptExpPayoff.append(get_call_price_at_expiration(d1,d2,St,int(k)))
...
for k in putOptStrike:
    for St in simulatedPrice:
        putOptPayoff.append(greek.get_price_delta(St,int(k),t,T,rf,sigma,'P',div))
