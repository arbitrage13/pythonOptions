...
div = 0.025
multiplier = 1
str = raw_input("please identify your broker: ")
#======ChartVariables=====#
...
def fileLoad(broker):
    if(broker == "cgs"):
        brokerTxt = os.path.abspath('CGS/optionCGS_1Q2019.txt')
        with open(brokerTxt, 'r') as input, open(os.path.abspath('CGS/optionCGS_1Q2019.csv'),'w') as output:
        ...
        filename = os.path.abspath('CGS/optionCGS_1Q2019.csv')
        ...
        brokerTxt = os.path.abspath('DBSV/optionDBSV_1Q2019.txt')
        with open(brokerTxt, 'r') as input, open(os.path.abspath('DBSV/optionDBSV_1Q2019.csv'),'w') as output:
            non_comma = (line.replace(",","") for line in input if line.strip())
            non_tab = (line.replace('\t', ',') for line in non_comma if line.strip())
            output.writelines(non_tab)

        filename = os.path.abspath('DBSV/optionDBSV_1Q2019.csv')
        df = pd.read_csv(filename,index_col=False)
        ...
        brokerTxt = os.path.abspath('AUS/optionAUS_1Q2019.txt')
        with open(brokerTxt, 'r') as input, open('optionAUS_1Q2019.csv','w') as output:
            non_comma = (line.replace(",","") for line in input if line.strip())
            non_tab = (line.replace('\t', ',') for line in non_comma if line.strip())
            output.writelines(non_tab)

        filename = 'optionAUS_1Q2019.csv'
        df = pd.read_csv(filename,index_col=False)
        df['Contracts'] = np.where(df['BuySell'] == 'S' , df['Contract'] *-1 ,df['Contract'])
        df['Type'] = np.where(df['Series'].str.len() > 6, 'OPT', 'STK')
        df['Strike'] = np.where(df['Type'] == 'OPT',df['Series'].str[7:11],"")
